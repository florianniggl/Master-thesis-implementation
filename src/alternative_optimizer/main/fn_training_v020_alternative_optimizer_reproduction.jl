# ---------------------------------------------------------------------------------------------------------------------
# fn_training_v020_alternative_optimizer_reproduction.jl is a MODIFICATION of training.jl from GeometricMachineLearning@v0.2.0
...
#   - system parameters are set in separate file fn_global_consts.jl (included in subproject reproduction)
...
println("1D WAVE EQUATION: ALTERNATIVE OPTIMIZER TRAINING")
println()
...
include("../../reproduction/general/fn_assemble_matrix_v020.jl")
include("../../reproduction/general/fn_vector_fields_v020.jl")
include("../../reproduction/general/fn_initial_condition_v020.jl")
include("../../reproduction/general/fn_global_consts.jl")
...
elseif StiefelCayleyAdamOptimizerFromOtherPaper_global
    opt = StiefelCayleyAdamOptimizerFromOtherPaper(T.((0.4, 0.9, 0.9, 0.9, 0.9, 1e-8, 1.0, 0.5, 2))...)
...
if TEST_global
    ...
    name_errors = "alternative_optimizer_errors_N=$(N_global)_TEST"
else
    ...
    name_errors = "alternative_optimizer_errors_N=$(N_global)_$(n_range_string)"
end

plot_name = "alternative_optimizer_reconstructed_NN"
...
function get_nn_encoder_decoder(; n=5, n_epochs=500, activation=tanh, opt=opt, T=T, T_test=T_test)
    # Reset that step length η to inital value for each new encoder/decoder learning (important for decay optimizers)
    ...
    if epochwise_global
        ...
        for _ in 1:n_training_iterations
            ...
            loss_val, pb =  
                Zygote.pullback(ps -> loss(model, ps, input_batch, datatype), ps)
            dp = pb(one(loss_val))[1]
            try 
                optimization_step!(optimizer_instance, met, vt, model, ps, dp)
            catch
                continue 
            end
