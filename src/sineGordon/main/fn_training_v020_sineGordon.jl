...
println("1D SINE GORDON EQUATION: TRAINING")
println()
...
opt = AdamOptimizer(sG.T_analytic.((0.001, 0.9, 0.99, 1e-8))...)
retraction = Cayley()
...
if TEST_global
    ...
    name_errors = "$(example)_errors_N=$(N_global)_TEST"
else
    ...
    name_errors = "$(example)_errors_N=$(N_global)_$(n_range_string)_epochwise"
end

plot_name = "reconstructed_NN_epochwise"
...
# load precalculated PSD-errors (always unnormalized as we do not want to change the comparison method)
